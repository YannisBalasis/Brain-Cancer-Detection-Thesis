#!/usr/bin/env python3
"""
Training script for EfficientNet-B3 Dual Branch System
Implements optimized 3-phase training strategy
"""
...
# Import our modules
from efnet_dual_system import EfficientNetDualBranchSystem, create_efnet_dual_system_model
...
from efnet_dual_system_config import (
    get_config, get_paths_config, 
    TRAINING_CONFIG, CALLBACKS_CONFIG,
    REPRODUCIBILITY_CONFIG
)

class EfficientNetDualSystemTrainer:
    """
    Main trainer class για EfficientNet-B3 Dual Branch System
    """
    ...
    def __init__(self, data_path, config=None):
        ...
        self.logger.info("EfficientNet-B3 Dual System Trainer initialized")
    ...
    def setup_experiment(self):
        ...
        # Set random seeds για reproducibility
    ...
    def build_model(self):
        """
        Build EfficientNet dual system model
        """
        self.logger.info("Building EfficientNet-B3 dual system model...")
        
        self.efnet_dual_system, self.model = create_efnet_dual_system_model()
        
        # Log model summary
        summary_info = self.efnet_dual_system.get_model_summary()
        self.logger.info("Model built successfully")
        self.logger.info(f"Total parameters: {summary_info['total_params']:,}")
        self.logger.info(f"Backbone: {summary_info['backbone']}")
        self.logger.info(f"Fusion strategy: {summary_info['fusion_strategy']}")
        
        # Save model summary to file
        summary_file = os.path.join(self.paths['RESULTS_PATH'], 'efnet_model_summary.txt')
    ...
    def _create_callbacks(self, phase, model_save_path):
        """
        Create training callbacks για specific phase
        ...
        self.logger.info(f"Callbacks created για {phase}")
        return callbacks
    
    def train_phase1(self):
        """
        Phase 1: Train with frozen EfficientNet-B3
        """
        phase_name = "Phase 1: Frozen EfficientNet-B3"
        self.logger.info(f"Starting {phase_name}")
        
        # Ensure EfficientNet is frozen
        self.efnet_dual_system.freeze_efficientnet_layers()
        
        # Compile model για phase 1
        self.efnet_dual_system.compile_model('phase1')
        ...
        # Save phase history
        history_file = os.path.join(self.paths['RESULTS_PATH'], 'efnet_phase1_history.json')
        with open(history_file, 'w') as f:
            json.dump(history.history, f, indent=2)
    
    def train_phase2(self):
        """
        Phase 2: Fine-tune EfficientNet-B3 layers
        """
        phase_name = "Phase 2: EfficientNet-B3 Fine-tuning"
        self.logger.info(f"Starting {phase_name}")
        
        # Unfreeze EfficientNet layers
        unfreeze_from = self.config['model']['EFFICIENTNET_B3']['UNFREEZE_FROM_LAYER']
        self.efnet_dual_system.unfreeze_efficientnet_layers(unfreeze_from)
        
        # Compile model για phase 2 (lower learning rate)
        self.efnet_dual_system.compile_model('phase2')
        ...
        # Save phase history
        history_file = os.path.join(self.paths['RESULTS_PATH'], 'efnet_phase2_history.json')
        with open(history_file, 'w') as f:
            json.dump(history.history, f, indent=2)
    ...
    def train_phase3(self, enable_phase3=True):
        ...
        # All layers should already be unfrozen από phase 2
        # Compile model για phase 3 (very low learning rate)
        self.efnet_dual_system.compile_model('phase3')
        ...
        # Save phase history
        history_file = os.path.join(self.paths['RESULTS_PATH'], 'efnet_phase3_history.json')
        with open(history_file, 'w') as f:
            json.dump(history.history, f, indent=2)
    
    def train_complete_system(self, enable_phase3=True):
        """
        Train the complete EfficientNet dual system through all phases
        
        Args:
            enable_phase3: Whether to run the optional phase 3
        """
        self.logger.info("Starting complete EfficientNet dual system training")
    ...
    def _save_best_model(self):
        ...
        if best_phase:
            ...
            if os.path.exists(source_path):
                ...
                self.logger.info(f"Best model από {best_phase} saved")
    ...
    def _create_training_visualizations(self):
        ...
        plot_path = os.path.join(self.paths['PLOTS_PATH'], 'efnet_training_history.png')
        self.visualizer.plot_training_history(combined_history, plot_path)
        
        # Individual phase plots
        for phase, data in self.training_phases.items():
            phase_plot_path = os.path.join(self.paths['PLOTS_PATH'], f'efnet_{phase}_history.png')
    ...
    def _save_training_summary(self):
        ...
        summary = {
            ...
            'model_info': self.efnet_dual_system.get_model_summary(),
            'training_phases': {}
        }
        ...
        # Save summary
        summary_file = os.path.join(self.paths['RESULTS_PATH'], 'efnet_training_summary.json')
        with open(summary_file, 'w') as f:
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Train EfficientNet-B3 Dual Branch System')
    ...
    # Print header
    print("=" * 80)
    print("EFFICIENTNET-B3 DUAL BRANCH SYSTEM TRAINING")
    print("=" * 80)
    ...
    try:
        # Initialize trainer
        trainer = EfficientNetDualSystemTrainer(args.data)
        ...
        print("=" * 80)
        print("EFFICIENTNET-B3 TRAINING COMPLETED SUCCESSFULLY")
        print("=" * 80)
        ...
        print(f"Best performance: {best_acc:.4f} από {best_phase}")
        print("=" * 80)
